...
public class SecretTalkMessengerApplication extends Application {
   ...
   public synchronized DataAccessHelper getDataAccessHelper() {
   ...
   public static void addToApplicationLog(String message) {
      ...
      try {
         ...
         buffer.close();
			...
			Log.d(SecretTalkMessengerApplication.LOGKEY, "appending to AppLog: " + message);
      } catch (IOException e) { Log.d(LOGKEY, e.toString()); } 
   }
   ...
   public static long getLagToLastKeepAliveMarkerInMillis(String tag) {
      ...
      File file = new File(filename); 
      if (!file.exists()) {
         Log.d(SecretTalkMessengerApplication.LOGKEY, "getLagToLastKeepAliveMarkerInMillis: file does not exist");
         return -1;
      }
      try {
         ...
         if (splitResult.length == 2) {
            try {
               ...
               Log.d(SecretTalkMessengerApplication.LOGKEY, String.format("getLagToLastKeepAliveMarkerInMillis: number format exception, while converting %s", splitResult[1]));
               return -1;
            }
         } else {
            Log.d(SecretTalkMessengerApplication.LOGKEY, String.format("getLagToLastKeepAliveMarkerInMillis: could not obtain millis when splitting <%s>", lastKeepAlive));
            return -1;
         }
      } catch (IOException e) { 
   ...
   public static void checkBackgroundService(Activity activity) {
      String currentmode = ((SecretTalkMessengerApplication)(activity.getApplication())).configHelper.getBackground();
      if (currentmode.equals(ConfigHelper.CONFIG_KEY_BACKGROUND_OPTION_WIFI) || currentmode.equals(ConfigHelper.CONFIG_KEY_BACKGROUND_OPTION_MOBILE)) {
         long lag_KeepAlive = SecretTalkMessengerApplication.getLagToLastKeepAliveMarkerInMillis(KeepAliveCheck.KEEPALIVECHECK_KEEPALIVE);
         ...
         long lag_PeriodicMessageCheck = SecretTalkMessengerApplication.getLagToLastKeepAliveMarkerInMillis(PeriodicMessageCheck.PERIODICMESSAGECHECK_KEEPALIVE);
